...
def box_iou(boxes1, boxes2):
    ...
    #     raise TypeError("Inputs 'boxes1' and 'boxes2' must be PyTorch tensors.")
    #
    #     # 检查输入数据形状
    # if boxes1.dim() != 2 or boxes1.size(1) != 4:
    #     raise ValueError("Input 'boxes1' must be a 2D tensor with shape (N, 4).")
    # if boxes2.dim() != 2 or boxes2.size(1) != 4:
    #     raise ValueError("Input 'boxes2' must be a 2D tensor with shape (N, 4).")

        # 检查输入数据内容
    if not torch.is_floating_point(boxes1) or not torch.is_floating_point(boxes2):
        raise TypeError("Elements in 'boxes1' and 'boxes2' must be floating point numbers.")
    """
