...
class RoIHeads(torch.nn.Module):
    ...
    def subsample(self, labels):#返回正样本的索引值
    ...
    def select_training_samples(self,
        ...
        # print(" ")
        #labels中为0的是负样本（iou没达到要求，有可能是裂痕或者噪声）
    ...
    def forward(self,
                ...
                # standard_feature = None,
                # proposals_stand = None,
                # image_shapes_stand = None,
                # targets_stand = None
                ):
        ...
        with torch.no_grad():
            ...
            # pred_boxes = self.box_coder.decode(box_regression, proposals)
            # class_logits = F.sigmoid(class_logits)
            # box_regression = F.sigmoid(box_regression)
            # print()
            # class_logits = F.sigmoid(self.box_predictor(box_features1))[0]
            # box_regression = F.sigmoid(self.box_predictor(box_features1))[1]
            ...
            # proposals_stack = torch.cat(proposals, dim=0)
            # proposals_c = proposals_stack[proposal_id]
            # pro_batch = []
            # num = 0
            ...
            #         if num+n in proposal_id_list:
            #             list.append(m[n].tolist())
            #     num = num + m.shape[0]
            #     list_array = np.array(list)
            #     pro_batch.append(list)

            # for m in proposals:
            #
            #     list = []
            #     for n in range(m.shape[0]):
            #         list.append(proposals_c[num+n].tolist())
            #     num = num + m.shape[0]
            #     list_array = np.array(list)
            #     pro_batch.append(list)
